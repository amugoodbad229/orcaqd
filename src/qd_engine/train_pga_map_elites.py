...
def make_scoring_fn(env: DexHandEnv, policy_net: PolicyNetwork, cfg: TrainConfig):
    ...
    def score_one_policy(params, key):
        ...
        def rollout_one_seed(seed_key):
            ...
            # jax.checkpoint: recompute activations during backprop instead of
            # storing them. Saves large amounts of GPU memory when PG critic
            # gradients are computed through the 250-step rollout.
            @jax.checkpoint
            def step_fn(carry, _):
